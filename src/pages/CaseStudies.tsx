      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10 mb-20">
          <div className="space-y-6">
            <div className="font-mono text-[10px] tracking-[0.5em] text-accent flex items-center gap-4">
              <Quote size={12} />
          ...
          <div className="flex gap-4">
            <button 
              onClick={prev}
              className="w-12 h-12 border border-white/10 flex items-center justify-center hover:bg-accent hover:border-accent transition-all group rounded-full"
            >
            ...
            >
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="font-mono">
                      <div className="text-sm tracking-widest text-white">{testimonials[currentIndex].author}</div>
                      <div className="text-[10px] tracking-widest opacity-40">{testimonials[currentIndex].role}, {testimonials[currentIndex].company}</div>
                    </div>
                  </div>
                  <div className="font-mono text-[8px] tracking-[0.3em] opacity-20 ml-14">
              ...
              <div className="lg:col-span-4 hidden lg:block">
                <div className="w-full aspect-square bg-white/[0.02] border border-white/5 flex items-center justify-center relative group">
                  ...
                  <div className="absolute bottom-6 left-6 font-mono text-[8px] tracking-widest opacity-20 uppercase">
                    Auth sig // {testimonials[currentIndex].author}
                  </div>
                </div>
              </div>
            </motion.div>
...
const CaseStudyCard = ({ title, sector, description, image, outcome, tags }: any) => (
  <div className="card-reveal-container mb-20 lg:mb-40">
    <div className="case-card-inner grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 p-8 md:p-16 lg:p-20 bg-ocean-900 border border-white/5 relative shadow-2xl backdrop-blur-3xl">
      <div className="space-y-8">
        ...
        <p className="font-mono text-sm tracking-widest leading-loose opacity-60 max-w-xl">
          {description}
        </p>
        
        <div className="space-y-6">
          <h4 className="font-mono text-[10px] tracking-widest text-accent font-bold">Protocol outcomes:</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {outcome.map((item: string, i: number) => (
              <div key={i} className="flex items-start gap-3 group">
                ...
                <span className="font-mono text-[10px] tracking-widest opacity-40 group-hover:opacity-100 transition-opacity">
                  {item}
                </span>
              </div>
            ))}
        ...
        <div className="flex flex-wrap gap-4 pt-10">
          {tags.map((tag: string) => (
            <span key={tag} className="px-5 py-2 border border-white/10 font-mono text-[8px] tracking-widest opacity-40 hover:opacity-100 hover:border-accent/40 transition-all cursor-default">
              {tag}
            </span>
          ))}
        </div>
      ...
      <div className="relative group overflow-hidden bg-ocean-950 border border-white/10 p-1 aspect-[4/5]">
         <img src={image} loading="lazy" decoding="async" className="case-reveal-img w-full h-full object-cover grayscale brightness-50 group-hover:grayscale-0 group-hover:brightness-75 group-hover:scale-105 transition-all duration-1000" alt={title} />
         <div className="absolute inset-0 bg-accent/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
         
         <div className="absolute top-8 right-8 p-4 bg-black/80 backdrop-blur-md border border-white/10 font-mono text-[8px] tracking-[0.3em] opacity-40">
            [ Deployment active ]
         </div>
      </div>
    </div>
...
export default function CaseStudies() {
  useEffect(() => {
    ...
    // GSAP Cards overlap
    const cards = gsap.utils.toArray<HTMLElement>('.card-reveal-container');
    cards.forEach((card, i) => {
      const inner = card.querySelector('.case-card-inner');
      const img = card.querySelector('.case-reveal-img');
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => gsap.to(inner, { boxShadow: '0 50px 100px -20px rgba(225,29,72,0.1)', duration: 0.5 }),
      });
  ...
  const cases = [
    ...
    {
      title: "Heavy Haulage Excellence",
      sector: "Inland Logistics",
      description: "Moving industrial equipment across West African corridors. We utilize modernized tracking and escort protocols for over-dimensional cargo.",
      image: truckImg,
      outcome: [
        "Zero transit safety incidents",
        "Real-time GPS tracking active",
        "Cross-border clearance managed",
        "Scheduled maintenance protocols"
      ],
      tags: ["Industrial Logistics", "Heavy Lift", "West Africa Corridor"]
    },
    {
  ...
  return (
    <motion.div 
      ...
      className="bg-ocean-950 min-h-screen relative selection:bg-white selection:text-black pt-32 px-6"
    >
      
      {/* Dynamic Header */}
      <div className="max-w-7xl mx-auto mb-32 space-y-12">
        <Link to="/" className="inline-flex items-center gap-3 font-mono text-[10px] tracking-widest text-accent group px-6 py-3 border border-accent/20 rounded-full hover:bg-accent/5 transition-all">
        ...
        <div className="space-y-6">
          ...
          <p className="font-mono text-xs lg:text-sm tracking-widest leading-loose opacity-40 max-w-2xl">
      ...
      {/* Footer Minimal */}
      <footer className="py-20 border-t border-white/5 opacity-30 text-center font-mono text-[8px] tracking-widest uppercase">
         [ Data end // Ocean masters int ]
      </footer>
    </motion.div>
  );
}
